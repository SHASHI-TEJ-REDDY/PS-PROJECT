...
async function connectMetamask() {
    if (window.ethereum !== "undefined") {
        ...
        account2 = accounts[0];
        document.getElementById("accountArea").innerHTML = `Account is: ${account2}`;
    }
}
...
const connectContract2 = async () => {
    ...
    const Address = "0xF77df31F834f6f9c50bC04F2F8807Baa7700Bc27";
    ...
    else if(window.location.pathname=="/verify56.html"){
        verifier();
    }
    else if(window.location.pathname=="/verif.html"){
        verified();
    }
...
let verifier = async () => {
    ...
    for (let patch of patches) {
        ...
        reject.onclick = async () => {
            contract.methods.rvadd(patch["patchfilename"]).send({ from: account }).then(() => {
                alert("Successfully Updated");
                window.location.reload();
            })
        }
...
const submitpr = async () => {
    await window.contract.methods.gprior(arr3).send({ from: account });
...
const check = async () => {
    ...
    await window.contract.methods.gbf(enabledSettings, enabledSettings2).send({ from: account });
...
const prequest = async () => {
    ...
    for (var i = 0; i < dt5[0].length; i++) {
        ...
        var cell2 = row.insertCell(1);
        // row.appendChild(x);
        cell1.innerHTML = dat[2];
        cell2.innerHTML = dat[4];
        // console.log(data56[i][3]);
        // x.addEventListener('change', function() {
        //     if (this.checked) {
            //         console.log("Checkbox is checked..");
            ...
            //     } else {
                ...
                for (var i = 0; i < dt5[0].length; i++) {
                    const dat = await window.contract.methods.gbug(dt5[1][i]).call();
                    // s += `${data45[0][i]}  `+`${data45[1][i]+"<br>"}`;
                    // var x = document.createElement("input");
                    // // x.id = x+i;
                    // x.setAttribute("type", "checkbox");
